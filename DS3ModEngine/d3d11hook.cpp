...
D3D11_HOOK_API bool ImplHookDX11_Init(HMODULE hModule, void *hwnd)
{
	...
	wprintf(L"[ModEngine] Hooking D3D Create Device\r\n");
	/*if (MH_CreateHookApi(L"D3D11", "D3D11CreateDevice", &tD3D11CreateDevice, reinterpret_cast<LPVOID*>(&fpD3D11CreateDevice)) != MH_OK)
		return false;

	if (MH_EnableHook((LPVOID)GetProcAddress(GetModuleHandleW(L"D3D11"), "D3D11CreateDevice")) != MH_OK)
		return false;

	if (MH_CreateHookApi(L"D3D11", "D3D11CreateDeviceAndSwapChain", &tD3D11CreateDeviceAndSwapChain, reinterpret_cast<LPVOID*>(&fpD3D11CreateDeviceAndSwapChain)) != MH_OK)
		return false;

	if (MH_EnableHook((LPVOID)GetProcAddress(GetModuleHandleW(L"D3D11"), "D3D11CreateDeviceAndSwapChain")) != MH_OK)
		return false;*/
